...
//gcc ppm_slave01.c  -I/opt/ethercat/include -lethercat -o ppm_slave01.bin
...
#define CopleySlavePos0    0, 0
#define CopleySlavePos1    0, 1
...
typedef struct{
    //read Roll,Pitch,Yaw from fifo.
    int fd_fifo;
}ZServoDevice;
ZServoDevice gServoDev;
...
{
    { CopleySlavePos0, Copley_VID_PID, 0x6040, 0, &ctrlWord[0], NULL },
    { CopleySlavePos0, Copley_VID_PID, 0x607A, 0, &targetPosition[0], NULL },

    { CopleySlavePos1, Copley_VID_PID, 0x6040, 0, &ctrlWord[1], NULL },
    { CopleySlavePos1, Copley_VID_PID, 0x607A, 0, &targetPosition[1], NULL },
    {}
};
const static ec_pdo_entry_reg_t domainInput_regs[]=
{
    { CopleySlavePos0, Copley_VID_PID, 0x6041, 0, &statusWord[0], NULL },
    { CopleySlavePos0, Copley_VID_PID, 0x606C, 0, &actVelocity[0], NULL },
    { CopleySlavePos0, Copley_VID_PID, 0x6064, 0, &actPosition[0], NULL },

    { CopleySlavePos1, Copley_VID_PID, 0x6041, 0, &statusWord[1], NULL },
    { CopleySlavePos1, Copley_VID_PID, 0x606C, 0, &actVelocity[1], NULL },
    { CopleySlavePos1, Copley_VID_PID, 0x6064, 0, &actPosition[1], NULL },
    {}
};
...
void cyclic_task()
{
    ...
    static int iLastTarPos=0;
    static int iReverseFlag=0;
    ...
    case SYS_WORKING_SAFE_MODE:
        ...
        {
            int tmp=true;
            if(slave_state[0].al_state!=ETHERCAT_STATUS_OP)
            {
                tmp=false;
            }
            if(slave_state[1].al_state!=ETHERCAT_STATUS_OP)
            {
                tmp=false;
            }
            if(tmp)
    ...
    case SYS_WORKING_OPMODE:
    {
        ...
        case 1://set Operation Mode.
            ...
            for(int i=0;i<2;i++)
            {
                ecrt_slave_config_sdo8(slave[i],0x6060,0,1);
            }
            iTickCnt++;
            break;
        ...
        case 2://Set Parameter.
            ...
            {
                //Sync Manager2,Synchronization Type=0:Free Run.
                ecrt_slave_config_sdo16(slave[i],0x1c32,0x01,0);

                //Following error window.
                ecrt_slave_config_sdo32(slave[i],0x6065,0x00,8015078);

                //Profile Velocity.
                ecrt_slave_config_sdo32(slave[i],0x6081,0x00,500000);
                //Profile Acceleration.
                ecrt_slave_config_sdo32(slave[i],0x6083,0x00,50000);
                //Profile Deceleration.
                ecrt_slave_config_sdo32(slave[i],0x6084,0x00,50000);
                //Quick Stop Deceleration.
                ecrt_slave_config_sdo32(slave[i],0x6085,0x00,10000);
                //Motion Profile Type=T.
                ecrt_slave_config_sdo16(slave[i],0x6086,0x00,0);
            }
            iTickCnt++;
        ...
        case 5://Set Target Position to 0 at start up.
            g_PPMPositionMode=PPM_POSITION_ABSOLUTE;
            EC_WRITE_S32(domainOutput_pd+targetPosition[0],0);
            EC_WRITE_S32(domainOutput_pd+targetPosition[1],0);
            iLastTarPos=0;
            //here we skip step 6 to step 7 directly at startup.
            iTickCnt++;
            iTickCnt++;
            break;

        case 6://Set Target Position.
        {
            int pos;
            if(iReverseFlag)
            {
            ...
            iReverseFlag=!iReverseFlag;
            printf("set new position:%d\n",pos);
            //g_PPMPositionMode=PPM_POSITION_ABSOLUTE;
            //EC_WRITE_S32(domainOutput_pd+targetPosition[0],pos);
            g_PPMPositionMode=PPM_POSITION_ABSOLUTE;
            EC_WRITE_S32(domainOutput_pd+targetPosition[0],pos);
            EC_WRITE_S32(domainOutput_pd+targetPosition[1],pos);
            iTickCnt++;
        }
            break;

        case 7://Start Positioning.
            ...
            iTickCnt++;
            break;
        case 8://set point acknowledge.
        {
            uint16_t status0,status1;
            status0 = EC_READ_U16(domainInput_pd + statusWord[0]);
            status1 = EC_READ_U16(domainInput_pd + statusWord[1]);
            if((status0&(0x1<<12)) && (status1&(0x1<<12)))
            {
                printf("set point acknowledge\n");
                iTickCnt++;
            }else{
                int curPos0=EC_READ_S32(domainInput_pd + actPosition[0]);
                int curPos1=EC_READ_S32(domainInput_pd + actPosition[1]);
                printf("set point not acknowledge:%d,%d.\n",curPos0,curPos1);

                if((iLastTarPos==curPos0) && (iLastTarPos==curPos1))
                {
                    printf("current pos= preset pos!\n");
                    iTickCnt++;
                }
            }
        }
...
int main(int argc, char **argv)
{
    ...
    //open fifo for reading Roll,Pitch,Yaw data.
    ...
    if(!(slave[0]=ecrt_master_slave_config( master, CopleySlavePos0, Copley_VID_PID)))
    {
        printf("error:failed to get slave configuration for Copley-0.\n");
        return -1;
    }
    if(!(slave[1]=ecrt_master_slave_config( master, CopleySlavePos1, Copley_VID_PID)))
    {
        printf("error:failed to get slave configuration for Copley-0.\n");
        return -1;
    }
    ...
    if(ecrt_slave_config_pdos(slave[0], EC_END, copley_syncs))
    {
        printf("error:failed to configure Copley PDOs.\n" );
        return -1;
    }
    if(ecrt_slave_config_pdos(slave[1], EC_END, copley_syncs))
    {
        printf("error:failed to configure Copley PDOs.\n" );
        return -1;
    }
...
void g_SignalHandler(int signo)
{
    if(signo==SIGINT)
    {
        fprintf(stdout,"SIGINT:request to exit\n");
        g_bRst2ExitFlag=1;
    }
}

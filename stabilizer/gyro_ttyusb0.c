...
typedef struct{
    ...
    struct Angle0x53 data_angle_old;
...
void parse_gyro_protocol(int fd_fifo,const char *buffer,int len)
{
#if 0
    printf("parse:\n");
    for(int i=0;i<len;i++)
    ...
    case 0x53:
    {
                ...
                (gyroDev.data_angle.z_yaw!=gyroDev.data_angle_old.z_yaw) )
        {
            ...
            gyroDev.data_angle_old.z_yaw=gyroDev.data_angle.z_yaw;
            gyroDev.data_angle_old.temperature=gyroDev.data_angle.temperature;


            sprintf(buffer2pipe,"R:%.2f,P:%.2f,Y:%.2f\n",///<
                    gyroDev.data_angle.x_roll,///<
                    gyroDev.data_angle.y_pitch,///<
                    gyroDev.data_angle.z_yaw);
            length=htonl(length);
            write(fd_fifo,&length,sizeof(length));
            write(fd_fifo,buffer2pipe,strlen(buffer2pipe));
        }
    }
        break;
    case 0x54:
...
void *consumer_thread(void *p)
{
    ...
    int i_total_bytes;
    int fsm=FSM_RESET;
    //open fifo for writing data.
    if((dev->fd_fifo=open(FIFO_NAME,O_RDWR))<0)
    {
        printf("error at open() fifo.\n");
        dev->bRst2ExitFlag=1;
        return 0;
    }
    while(!dev->bRst2ExitFlag)
        ...
        case FSM_SCAN_PROCESS:
        {
            ...
            while( (i_total_bytes-rd_pos)>=11 )
            {
                if( buffer[rd_pos]==0x55 && (buffer[rd_pos+1]>=0x50 && buffer[rd_pos+1]<=0x58) )
                {
                    ...
                    {
                        parse_gyro_protocol(dev->fd_fifo,&buffer[rd_pos+1],11);
                        rd_pos+=12;
                    }else{
                        parse_gyro_protocol(dev->fd_fifo,&buffer[rd_pos],11);
                        rd_pos+=11;
                    }
                }else{
                    rd_pos++;

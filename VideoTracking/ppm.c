...
//gcc ppm.c  -I/opt/ethercat/include -lethercat -o ppm.bin
    ...
    case SYS_WORKING_OPMODE:
    {
            ...
            if(read(g_fdServoFIFOIn,(void*)&len,sizeof(len))==sizeof(len))
            {
                ...
                if(read(g_fdServoFIFOIn,(void*)buffer,len)==len)
                {
                    ...
                    }else if(strstr(buffer,"rel_pos="))
                    {
                        ...
                        int s0_cur_pos=EC_READ_S32(domainInput_pd + actPosition[0]);
                        int s1_cur_pos=EC_READ_S32(domainInput_pd + actPosition[1]);
                        //minimum & maximum limit.
                        if((pos+s0_cur_pos)>=-20000 && (pos+s0_cur_pos)<=20000 && (pos2+s1_cur_pos)>=-20000 && (pos2+s1_cur_pos)<=20000)
                        {
                            EC_WRITE_S32(domainOutput_pd+targetPosition[0],pos);
                            EC_WRITE_S32(domainOutput_pd+targetPosition[1],pos2);
                        }else{
                            printf("relative reach boundary,abs_pos=%d,%d -> %d,%d\n",pos,pos2,pos+s0_cur_pos,pos2+s1_cur_pos);
                            bOverflow=1;
                        }
                    }
                    if(bOverflow)
        ...
        case 7://set point acknowledge.
        {
            ...
            if((status1&(0x1<<12)) && (status2&(0x1<<12)))
            {
                ...
                int s0_cur_pos=EC_READ_S32(domainInput_pd + actPosition[0]);
                int s1_cur_pos=EC_READ_S32(domainInput_pd + actPosition[1]);
                if(gTechServo.s0_login_zero==s0_cur_pos && gTechServo.s1_login_zero==s1_cur_pos)
                {
                    printf("Target already reached.\n");
                    iTickCnt=10;
                }
            }
        }
